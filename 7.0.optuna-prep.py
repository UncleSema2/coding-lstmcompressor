...
class RNNModel(nn.Module):
    """PyTorch implementation of the RNN model with configurable cell type."""

    def __init__(self, vocab_size: int, cell_type='lstm'):
        ...
        for i in range(num_layers):
            ...
            if self.cell_type == 'lstm':
                rnn_cell = nn.LSTM(input_size, rnn_units, batch_first=True)
            elif self.cell_type == 'gru':
                rnn_cell = nn.GRU(input_size, rnn_units, batch_first=True)
            elif self.cell_type == 'rnn':
                rnn_cell = nn.RNN(input_size, rnn_units, batch_first=True, nonlinearity='tanh')
            else:
                raise ValueError(f"Unknown cell_type: {self.cell_type}. Use 'lstm', 'gru', or 'rnn'")

            for name, param in rnn_cell.named_parameters():
                if 'weight' in name:
                    nn.init.xavier_uniform_(param)
            self.rnn_layers.append(rnn_cell)

        # Output layer
        output_size = rnn_units * num_layers if num_layers > 1 else rnn_units
        self.dense = nn.Linear(output_size, vocab_size)
        # Initialize dense layer weights to match TensorFlow
    ...
    def forward(self, inputs):
        ...
        # Skip connections will be used to connect each RNN layer output to the final
        # output layer. Each RNN layer will get as input both the original input and
        ...
        if self.cell_type == 'lstm':
            ...
            rnn_out, (hn, cn) = self.rnn_layers[0](embedded, (h0, c0))
            skip_connections.append(rnn_out)
            new_states.extend([hn.squeeze(0), cn.squeeze(0)])
        else:  # gru or rnn
            h0 = states[0].unsqueeze(0).contiguous()  # [1, batch_size, rnn_units]
            rnn_out, hn = self.rnn_layers[0](embedded, h0)
            skip_connections.append(rnn_out)
            new_states.append(hn.squeeze(0))
        ...
        for i in range(1, num_layers):
            ...
            if self.cell_type == 'lstm':
                ...
                rnn_out, (hn, cn) = self.rnn_layers[i](layer_input, (h0, c0))
                skip_connections.append(rnn_out)
                new_states.extend([hn.squeeze(0), cn.squeeze(0)])
            else:  # gru or rnn
                h0 = states[i].unsqueeze(0).contiguous()  # [1, batch_size, rnn_units]
                rnn_out, hn = self.rnn_layers[i](layer_input, h0)
                skip_connections.append(rnn_out)
                new_states.append(hn.squeeze(0))
...
class LSTMModel(RNNModel):
    """Backward compatibility wrapper for LSTM model."""
    def __init__(self, vocab_size: int):
        super(LSTMModel, self).__init__(vocab_size, cell_type='lstm')
...
def train(pos, seq_input, length, vocab_size, coder, model, optimizer, compress,
    ...
    mask_tensor = torch.tensor(mask, dtype=torch.float32, device=device).unsqueeze(1)
...
def process(compress, length, vocab_size, coder, data):
    ...
    start = time.time()
    reset_seed()
    model = RNNModel(vocab_size=vocab_size, cell_type=rnn_type).to(device)
    print(model)
    ...
    symbols_tensor = torch.tensor(symbols, dtype=torch.long, device=device)
    seq_input = symbols_tensor.unsqueeze(1).repeat(1, seq_length)  # [batch_size, seq_length]
    ...
    for j in range(num_layers):
        layer_states.append(torch.zeros(batch_size, rnn_units, device=device))  # hidden state
        if rnn_type.lower() == 'lstm':
            layer_states.append(torch.zeros(batch_size, rnn_units, device=device))  # cell state (LSTM only)
