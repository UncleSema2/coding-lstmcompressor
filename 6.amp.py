...
def train(pos, seq_input, length, vocab_size, coder, model, optimizer, compress,
    ...
    scaler.unscale_(optimizer)  # для gradient clipping
...
def process(compress, length, vocab_size, coder, data):
    ...
    for j in range(num_layers):
        ...
        layer_states.append(torch.zeros(batch_size, rnn_units, device=device))  # cell state
    ...
    while pos < split:
        seq_input, ce, d = train(pos, seq_input, length, vocab_size, coder, model,
                                 optimizer, compress, data, states, scaler)
        cross_entropy += ce
        denom += d
        pos += 1
        scheduler.step()
